...
const App = () => {
  ...
  // Hypothetical rates for scenario analysis
  const [hypotheticalRates, setHypotheticalRates] = useState({
    susdeApy: 7.74,
    ptSusdeImpliedApy: 12.5,
    morphoBorrowApy: 9.2,
    lltv: 0.75
  });
  ...
  const calculateMultiLoopEstimates = () => {
    const rates = useHypothetical ? hypotheticalRates : yieldRates;
    ...
    for (let i = 0; i < maxLoops && availableUSDC > 1; i++) {
      ...
      const maxBorrowUSDC = estimatedPTSUSDe * (rates.lltv || 0.75); // Use real LLTV or 75% default
    ...
    const totalPtYieldAnnual = (totalCollateral * rates.ptSusdeImpliedApy) / 100;
    const totalBorrowCostAnnual = (totalBorrowed * rates.morphoBorrowApy) / 100;
    ...
    return {
      ...
      usingHypothetical: useHypothetical
    };
  };
  ...
  const executeLoop = async (loopData, loopNumber) => {
    ...
    const stepActions = loopData.isLastLoop ? [
      ...
      () => simulateStep(3, loopNumber, `Swapping ${loopData.estimatedSUSDe.toFixed(2)} sUSDe → PT-sUSDe (Final Loop - No Borrowing)`, 1800)
    ] : [
      () => simulateStep(1, loopNumber, `Swapping ${loopData.inputUsdc.toFixed(2)} USDC → USDe`, 1500),
      () => simulateStep(2, loopNumber, `Staking ${loopData.estimatedUSDe.toFixed(2)} USDe → sUSDe`, 1200),
  ...
  const simulateStep44 = async (loopData, loopNumber) => {
    ...
    const collateralAmount = $u.moveDecimalRight(loopData.estimatedPTSUSDe.toString(), 18);
    const borrowAmount = Math.floor(parseFloat(loopData.actualBorrowAmount) * 1000000); // 6 decimals
    ...
    try {
      ...
      const isAuthorized = await morphoContract.isAuthorized(address, contracts.PT_SUSDE);
      if (!isAuthorized) {
        const authTx = await morphoContract.setAuthorization(contracts.PT_SUSDE, true);
        await authTx.wait();
      }
        ...
        <div className="text-center mb-8">
          <div className="flex items-center justify-center mb-4">
            ...
            <h1 className="text-4xl font-bold bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent">
              Multi-Loop DeFi Strategy
            </h1>
          </div>
          <p className="text-gray-300 text-lg">
            Leveraged yield farming with PT-sUSDe via Morpho and Pendle
          </p>
        </div>
            ...
            <div className="bg-slate-800/50 backdrop-blur-sm rounded-xl p-6 border border-purple-500/20">
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center">
                  ...
                  />
                  <label htmlFor="hypothetical" className="text-sm text-gray-400">
                    Use hypothetical rates for scenario analysis
                  </label>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-slate-800/50 backdrop-blur-sm rounded-xl p-6 border border-purple-500/20">
              <div className="flex items-center mb-4">
                ...
                <h3 className="text-xl font-semibold">
                  {useHypothetical ? 'Hypothetical' : 'Real-time'} Multi-Loop Estimates
                </h3>
              </div>
              ...
              <div className="mb-4">
                <div className="text-sm text-gray-400 mb-2">Annual Yield Breakdown ({useHypothetical ? 'Hypothetical' : 'Real'} Rates)</div>
                <div className="space-y-2">
                  <div className="flex justify-between">
                    <span>PT Yield ({useHypothetical ? hypotheticalRates.ptSusdeImpliedApy.toFixed(2) : (yieldRates.ptSusdeImpliedApy ? yieldRates.ptSusdeImpliedApy.toFixed(2) : '--')}%):</span>
                    <span className="text-green-400">+${multiLoopEstimates.totalPtYieldAnnual}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Borrow Cost ({useHypothetical ? hypotheticalRates.morphoBorrowApy.toFixed(2) : (yieldRates.morphoBorrowApy ? yieldRates.morphoBorrowApy.toFixed(2) : '--')}%):</span>
              ...
              <button
                ...
                disabled={isProcessing || !account || (!useHypothetical && !yieldRates.morphoBorrowApy)}
              ...
              >
                ...
                ) : (!useHypothetical && !yieldRates.morphoBorrowApy) ? (
            ...
            <div className="bg-slate-800/50 backdrop-blur-sm rounded-xl p-6 border border-purple-500/20">
              <div className="flex items-center mb-4">
                ...
                <h3 className="text-xl font-semibold">
                  Loop Breakdown ({useHypothetical ? 'Hypothetical' : 'Real'} Calculations)
                </h3>
              </div>
                  ...
                  <div key={index} className="bg-slate-700/50 rounded-lg p-3">
                    <div className="flex justify-between items-center mb-2">
                      <span className="font-semibold">
                        Loop {loop.loopNumber} {loop.isLastLoop ? '(Final - No Borrow)' : ''}
                      </span>
                    ...
                    <div className="text-sm space-y-1">
                      ...
                      {!loop.isLastLoop ? (
                        <>
                          ...
                          <div className="flex justify-between">
                            <span className="text-gray-400">Max at {((useHypothetical ? hypotheticalRates.lltv : yieldRates.lltv || 0.75) * 100).toFixed(0)}% LTV:</span>
                            <span>${loop.maxBorrowUSDC.toFixed(2)}</span>
                          </div>
                        </>
                      ) : (
                        <div className="text-purple-400 text-sm italic">
                          Final loop - Only supplying collateral
                        </div>
                      )}
                    </div>
                  </div>
              ...
              <div className="bg-slate-800/50 backdrop-blur-sm rounded-xl p-6 border border-orange-500/20">
                ...
                <div className="grid grid-cols-2 gap-4 mb-4">
                  <div className="bg-slate-700/50 rounded-lg p-3">
                    <div className="text-sm text-gray-400">Break-even Borrow Rate</div>
                    <div className="text-lg font-bold text-yellow-400">
                      {hypotheticalRates.ptSusdeImpliedApy.toFixed(2)}%
                    </div>
                    <div className="text-xs text-gray-500 mt-1">
                      Rate where net yield = 0
                    </div>
                  </div>
                  
                  <div className="bg-slate-700/50 rounded-lg p-3">
                    <div className="text-sm text-gray-400">Risk Buffer</div>
                    <div className={`text-lg font-bold ${
                      (hypotheticalRates.ptSusdeImpliedApy - hypotheticalRates.morphoBorrowApy) > 2 
                        ? 'text-green-400' 
                        : (hypotheticalRates.ptSusdeImpliedApy - hypotheticalRates.morphoBorrowApy) > 0
                        ? 'text-yellow-400'
                        : 'text-red-400'
                    }`}>
                      {(hypotheticalRates.ptSusdeImpliedApy - hypotheticalRates.morphoBorrowApy).toFixed(2)}%
                    </div>
                    <div className="text-xs text-gray-500 mt-1">
                      Yield spread cushion
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className={`font-medium ${
                      parseFloat(multiLoopEstimates.netYieldAnnual) > 50 ? 'text-green-400' :
                      parseFloat(multiLoopEstimates.netYieldAnnual) > 0 ? 'text-yellow-400' : 'text-red-400'
                    }`}>
                      {parseFloat(multiLoopEstimates.netYieldAnnual) > 50 ? 'Excellent' :
                       parseFloat(multiLoopEstimates.netYieldAnnual) > 0 ? 'Profitable' : 'Unprofitable'}
                    </span>
                  </div>
                  
                  <div className="flex justify-between">
                    <span className="text-gray-400">Leverage Risk:</span>
                    <span className={`font-medium ${
                      parseFloat(multiLoopEstimates.leverageMultiplier) < 2 ? 'text-green-400' :
                      parseFloat(multiLoopEstimates.leverageMultiplier) < 3 ? 'text-yellow-400' : 'text-red-400'
                    }`}>
                      {parseFloat(multiLoopEstimates.leverageMultiplier) < 2 ? 'Low' :
                       parseFloat(multiLoopEstimates.leverageMultiplier) < 3 ? 'Moderate' : 'High'}
                    </span>
                  </div>
                  
                  <div className="flex justify-between">
                    <span className="text-gray-400">LTV Safety:</span>
                    <span className={`font-medium ${
                      hypotheticalRates.lltv < 0.7 ? 'text-green-400' :
                      hypotheticalRates.lltv < 0.8 ? 'text-yellow-400' : 'text-red-400'
                    }`}>
                      {hypotheticalRates.lltv < 0.7 ? 'Conservative' :
                       hypotheticalRates.lltv < 0.8 ? 'Moderate' : 'Aggressive'}
                    </span>
                  </div>
                </div>
                ...
                <div className="mt-4 p-3 bg-blue-900/20 border border-blue-500/30 rounded-lg">
                  <div className="text-sm text-blue-300">
                    ...
                    <ul className="text-xs space-y-1 list-disc list-inside">
                      <li>Higher PT yield = better strategy performance</li>
                      <li>Lower borrow rates = higher net returns</li>
                      <li>More loops = higher leverage & risk</li>
                      <li>Conservative LTV = safer but lower returns</li>
                    </ul>
                  </div>
                </div>
              </div>
                    ...
                    <div key={index} className="bg-green-600/10 border border-green-600/20 rounded-lg p-3">
                      <div className="flex justify-between items-center">
                        <span className="font-semibold text-green-400">
                          Loop {loop.loopNumber} ✓ {loop.isLastLoop ? '(Final)' : ''}
                        </span>
                        <span className="text-sm text-gray-400">
                      ...
                      <div className="text-sm text-gray-300 mt-1">
                        Supplied ${loop.estimatedPTSUSDe.toFixed(2)} PT-sUSDe{loop.isLastLoop ? ' - No borrowing' : `, Borrowed ${loop.actualBorrowAmount.toFixed(2)} USDC`}
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="text-sm text-gray-300 space-y-2">
                ...
                {useHypothetical && (
                  <p className="font-semibold text-orange-400">
                    You are viewing hypothetical calculations. Real market conditions may differ significantly.
                  </p>
                )}
                <p className="font-semibold text-red-400">
