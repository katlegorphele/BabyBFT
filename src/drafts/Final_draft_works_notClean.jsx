...
const simulateStep1 = async (loopData, loopNumber) => {
  ...
  const simulateStep2 = async (loopData, loopNumber) => {
    ...
    try {
      ...
      setBalances(prev => ({ ...prev, susde: loopData.estimatedSUSDe.toString() }));
    } catch (error) {
  ...
  const simulateStep33 = async (loopData, loopNumber) => {
    setCurrentStep(3);
    ...
    const amountInWei = $u.moveDecimalRight(loopData.estimatedSUSDe.toString(), 18);
    const CHAIN_ID = 1;
    ...
    try {
      ...
      setBalances(prev => ({ ...prev, ptSusde: loopData.estimatedPTSUSDe.toString() }));
    } catch (error) {
  ...
  // Step 3: Swap sUSDe to PT-sUSDe via Pendle
  // Step 3: Swap sUSDe to PT-sUSDe via Pendle (Fixed version)
// Enhanced debugging version to identify the approval issue
const simulateStep3 = async (loopData, loopNumber) => {
  ...
  console.log('Amount to swap (decimal):', loopData.estimatedSUSDe.toString());
  console.log('Amount to swap (wei):', amountInWei.toString());
  console.log('sUSDe contract:', contracts.SUSDE);

  try {
    // First, let's check sUSDe balance
    ...
    console.log('sUSDe balance:', susdeBalance.toString());
    console.log('sUSDe balance (formatted):', ethers.utils.formatEther(susdeBalance));
    
    if (susdeBalance.lt(amountInWei)) {
      throw new Error(`Insufficient sUSDe balance. Have: ${ethers.utils.formatEther(susdeBalance)}, Need: ${ethers.utils.formatEther(amountInWei)}`);
    }
    ...
    console.log('Token approvals needed:', res.tokenApprovals);
    console.log('Transaction data:', res.data.tx);

    // Check what spender address we need to approve
    ...
    console.log('Current allowance (formatted):', ethers.utils.formatEther(currentAllowance));
    ...
    if (currentAllowance.lt(amountInWei)) {
      ...
      // Use maximum approval to avoid future approval issues
      const maxApproval = ethers.constants.MaxUint256;
      console.log('Approving maximum amount:', maxApproval.toString());
      
      setStatus('⏳ Approving sUSDe tokens...');
      
      // Reset allowance to 0 first (required for some tokens)
      ...
      setStatus(`⏳ Approval transaction sent: ${approvalTx.hash.substring(0, 10)}...`);
      ...
      if (newAllowance.lt(amountInWei)) {
        throw new Error(`Approval verification failed. Set: ${newAllowance.toString()}, Required: ${amountInWei.toString()}`);
      }
    ...
    // Wait a bit before executing the swap
    ...
    // Double-check allowance right before swap
    const finalAllowance = await susdeContract.allowance(address, spenderAddress);
    console.log('Final allowance check:', finalAllowance.toString());
    
    if (finalAllowance.lt(amountInWei)) {
      throw new Error(`Final allowance check failed. Have: ${finalAllowance.toString()}, Need: ${amountInWei.toString()}`);
    }
    ...
    try {
      ...
      console.error('Gas estimation still failed:', gasError);
      console.log('Using manual gas limit...');
      txParams.gasLimit = "800000"; // Higher gas limit
    }
    ...
    setStatus(`⏳ Swap transaction sent: ${tx.hash.substring(0, 10)}... Waiting for confirmation...`);
    ...
    if (error.message.includes('insufficient allowance')) {
      ...
      setStatus("❌ Insufficient sUSDe balance");
...
const simulateStep4 = async (loopData, loopNumber) => {
  ...
  try {
    ...
    setStatus(`✅ Loop completed! Supplied ${availablePT.toFixed(2)} PT-sUSDe, borrowed ${(borrowAmount / 1000000).toFixed(2)} USDC`);
    setCurrentStep(0);
